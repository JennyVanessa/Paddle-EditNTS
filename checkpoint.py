...
class Checkpoint(object):
    ...
    def save(self, experiment_dir):
        ...
        if os.path.exists(path):
            shutil.rmtree(path)
        os.makedirs(path)
        paddle.save({'epoch': self.epoch,
                    'step': self.step,
                    'opt': self.opt
                   },
                   os.path.join(path, self.TRAINER_STATE_NAME))
        paddle.save(self.model, os.path.join(path, self.MODEL_NAME))
    ...
    def load(cls, path):
        ...
        resume_checkpoint = paddle.load(os.path.join(path, cls.TRAINER_STATE_NAME))
        model = paddle.load(os.path.join(path, cls.MODEL_NAME), map_location=lambda storage, loc: storage)
