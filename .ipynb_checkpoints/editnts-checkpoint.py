...
import data
# from paddle.nn.RNN import pack_padded_sequence as pack
# from paddle.nn.RNN import pad_packed_sequence as unpack
...
def unsort(x_sorted, sorted_order):
    x_unsort = paddle.zeros_like(x_sorted)
    x_unsort[:, sorted_order,:] = x_sorted
    return x_unsort

def pack(input, lengths, batch_first=False, enforce_sorted=True):
    lengths = paddle.to_tensor(lengths, dtype='int64')
   

    data, batch_sizes = pack_padded_sequence(input, lengths, batch_first)
    return _packed_sequence_init(data, batch_sizes, sorted_indices, None)

def unpack(sequence, batch_first=False, padding_value=0.0, total_length=None):
...
class EncoderRNN(nn.Layer):
    def __init__(self, vocab_size, embedding_dim, pos_vocab_size, pos_embedding_dim,hidden_size, n_layers=1, embedding=None, embeddingPOS=None,dropout=0.3):
        ...
        self.rnn = nn.LSTM(embedding_dim+pos_embedding_dim, hidden_size, num_layers=n_layers, direction='bidirectional')
    ...
    def forward(self, inp, inp_pos, hidden):
        ...
        embed_cat = paddle.concat((emb,emb_pos),axis=2)
        packed_emb = paddle.nn.Embedding(embed_cat, inp_lengths_sorted)
    ...
    def initHidden(self, bsz):
        weight = next(iter(self.parameters())).data
        num_type = weight.dtype
        return paddle.zeros(shape=[self.n_layers * 2, bsz, self.hidden_size], dtype=num_type), \
               paddle.zeros(shape=[self.n_layers * 2, bsz, self.hidden_size], dtype=num_type)


class EditDecoderRNN(nn.Layer):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        ...
        if embedding is None:
            ...
            self.embedding = embedding
        self.rnn_edits = nn.LSTM(embedding_dim, hidden_size, num_layers=n_layers)
        self.rnn_words = nn.LSTM(embedding_dim, hidden_size, num_layers=n_layers)
        self.attn_Projection_org = nn.Linear(hidden_size, hidden_size, bias_attr=None)
    ...
    def execute(self, symbol, input, lm_state):
        ...
        is_keep = paddle.equal(symbol, data.KEEP_ID)
        is_del = paddle.equal(symbol, data.DEL_ID)
        if is_del:
            return lm_state
    ...
    def execute_batch(self, batch_symbol, batch_input, batch_lm_state):
        ...
        bsz = batch_symbol.shape[0]
        unbind_new_h = []
        unbind_new_c = []

        # unbind all batch inputs
        unbind_symbol = paddle.unbind(batch_symbol,dim=0)
        unbind_input = paddle.unbind(batch_input,dim=0)
        unbind_h = paddle.unbind(batch_h,dim=1)
        unbind_c = paddle.unbind(batch_c,dim=1)
        for i in range(bsz):
        ...
        new_batch_lm_h = paddle.concat(unbind_new_h,axis=1)
        new_batch_lm_c = paddle.concat(unbind_new_c,axis=1)
    ...
    def forward(self, input_edits, hidden_org,encoder_outputs_org, org_ids, simp_sent,teacher_forcing_ratio=1.):
        ...
        if use_teacher_forcing:
            ...
            logits_org = paddle.tensor.bmm(key_org, encoder_outputs_org.transpose(1, 2))  # bsz x nsteps x encsteps
            attn_weights_org = F.softmax(logits_org, dim=-1)  # bsz x nsteps x encsteps
            attn_applied_org = paddle.tensor.bmm(attn_weights_org, encoder_outputs_org)  # bsz x nsteps x nhid
            ...
            for t in range(nsteps-1):
                ...
                ## find current word
                inds = paddle.to_tensor(counter_for_keep_del)
                dummy = inds.view(-1, 1, 1)
                dummy = dummy.expand(dummy.shape[0], dummy.shape[1], encoder_outputs_org.shape[2]).cuda()
                c = encoder_outputs_org.gather(1, dummy)

                inds = paddle.to_tensor(counter_for_keep_ins)
                dummy = inds.view(-1, 1, 1)
                dummy = dummy.expand(dummy.shape[0], dummy.shape[1], output_words.shape[2]).cuda()
                c_word = output_words.gather(1, dummy)

                output_t =paddle.concat((decoder_output_t, attn_applied_org_t, c,c_word),
                ...
                check1 = sum([x >= org_ids.shape[1] for x in counter_for_keep_del])
                check2 = sum([x >= simp_sent.shape[1] for x in counter_for_keep_ins])
                if check1 or check2:
                    # print(org_ids.shape[1])
            ...
            t, tt = 0, max(MAX_LEN,input_edits.shape[1]-1)
            ...
            while t < tt:
                ...
                attn_applied_org_t = paddle.tensor.bmm(attn_weights_org_t, encoder_outputs_org)  # bsz x nsteps x nhid
                ...
                output_t =paddle.concat((output_edits, attn_applied_org_t, c, hidden_words[0]),
                                     2)  # bsz*nsteps x nhid*2
                output_t = self.attn_MLP(output_t)
                output_t = F.log_softmax(self.out(output_t), dim=-1)

                decoder_out.append(output_t)
                decoder_input_edit=paddle.argmax(output_t,dim=2)



                # gold_action = input[:, t + 1].vocab_data.cpu().numpy()  # might need to realign here because start added
                pred_action= paddle.argmax(output_t,dim=2)
                ...
                t += 1
                check = sum([x >= org_ids.shape[1] for x in counter_for_keep_del])
                if check:
                    break
        return paddle.concat(decoder_out, axis=1), hidden_edits

    def initHidden(self, bsz):
        weight = next(self.parameters()).data
        return weight.new(self.n_layers, bsz, self.hidden_size).zero_(), weight.new(self.n_layers, bsz, self.hidden_size).zero_()
    ...
    def beam_forwad_step(self,decoder_input_edits,hidden_edits,hidden_words, org_ids,encoder_outputs_org,counter_for_keep_del,beam_size=5):
        ...
        output_t = paddle.concat((decoder_output_t, attn_applied_org_t, c, hidden_words[0]),
                             2)  # bsz*nsteps x nhid*2
        output_t = self.attn_MLP(output_t)
        output_t = F.log_softmax(self.out(output_t), dim=-1)
        ...
        # find previous generated word
        # give previous word from tgt simp_sent
        topv, topi = paddle.topk(output_t,beam_size, dim=2)
    ...
    def beam_forward(self, input_edits, simp_sent, hidden_org,encoder_outputs_org, org_ids, beam_size=5):
        ...
        while t < tt:
            ...
            for b in range(len(best_k_seqs)):
                ...
                check = sum([x >= org_ids.shape[1] for x in counter])
...
class EditNTS(nn.Layer):
    def __init__(self, config, n_layers=2):
        ...
        if not(config.pretrained_embedding is None):
            ...
            self.embedding.weight.data = paddle.to_tensor(config.pretrained_embedding).clone()
    ...
    def forward(self,org,output,org_ids,org_pos,simp_sent,teacher_forcing_ratio=1.0):
        def transform_hidden(hidden): #for bidirectional encoders
            h, c = hidden
            h = paddle.concat([h[0], h[1]], axis=1)[None, :, :]
            c = paddle.concat([c[0], c[1]], axis=1)[None, :, :]
            hidden = (h, c)
            return hidden
        hidden_org = self.encoder1.initHidden(org[0].shape[0])
