...
def sort_by_lens(seq, seq_lengths):
    seq_len = np.array(seq_lengths)
    sort_order = paddle.to_tensor(np.argsort(seq_len)[::-1])
    seq_lengths_sorted = seq_lengths.sort(descending=True)
    seq_sorted = seq.index_select(sort_order, 0)
...
def reweigh_batch_loss(target_id_bath):
    ...
    NLL_weight_t = paddle.to_tensor(NLL_weight)
...
def training(edit_net,nepochs, args, vocab, print_every=100, check_every=500):
    ...
    editnet_optimizer = paddle.optimizer.Adam(learning_rate=1e-3, parameters=edit_net.parameters(),weight_decay=1e-6)
    ...
    for epoch in range(nepochs):
        ...
        for i, batch_df in train_dataset.batch_generator(batch_size=args.batch_size, shuffle=True):
            ...
            org_lens = org_ids.not_equal(paddle.zeros_like(org_ids)).sum(1).astype('int')
            ...
            org_pos_lens = org_pos_ids.not_equal(paddle.zeros_like(org_pos_ids)).sum(1).astype('int')
            ...
            editnet_optimizer.clear_grad()
            output = edit_net(org, out, org_ids, org_pos,simp_ids)
            ##################calculate loss
            tar_lens = tar.not_equal(paddle.zeros_like(tar)).sum(1).float().astype('int')
            ...
            paddle.nn.ClipGradByNorm(edit_net.parameters(), 1.)
...
def main():
    ...
    #train_file = 'data/%s/train.df.filtered.pos'%dataset
    # test='data/%s/test.df.pos' % args.dataset
    ...
    print('*'*10)
    vocab = data.Vocab()
    vocab.add_vocab_from_file('vocab_data/vocab.txt', args.vocab_size)
